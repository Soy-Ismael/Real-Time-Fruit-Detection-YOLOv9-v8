...
class FruitDetector:
    ...
    def __init__(self, config_path='dev/config.yaml'):
        ...
        #* Checkear esto, seria buena idea utilizar directamente self.config.x en lugar de declararlo individualmente
        #! Checkear esto, seria buena idea utilizar directamente self.config.x en lugar de declararlo individualmente
    ...
    # Añadir un bloque try-catch en esta funcion
    # def inference(self, img, model, confidence=0.60, gpu=False, local=True, save_image_debug_path=None):
    def inference(self, img, model=None, confidence=None, gpu=None, local=True, save_image_debug_path=None):
        ...
        if gpu:
            local_model = YOLO(model).to('cuda')
        else:
            local_model = YOLO(model).to('cpu')
        ...
        # if save_debug_img:
            # result[0].save(f'/var/www/fruit.homis.duckdns.org/dev/web/inferenced/test{self.index}.png')
            # pass
        ...
        #     print('json', detection.tojson())  # Print detection boxes
        if local:
            result[0].show() # Genera una imagen con las detecciones visualizadas
        else:
            return {'image': result[0].plot(), 'json': result[0].tojson()} # Genera una imagen con las detecciones visualizadas
